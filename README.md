...
Real problems hit and fixed during the build, not a wishlist — picked for spread across the stack rather than completeness (the model diagnosis and the confidence-calibration finding each get their own full writeup above and aren't repeated here).

- **A non-stationary regression target silently wrecked the model (test R² of -11.40, worse than predicting the mean).** The fix (target the *change* in cash position, not its raw cumulative value) and the full diagnosis are in "Measured model performance" above — the short version is that a chronological train/test split put numerically disjoint ranges in each half, and the model had no way to extrapolate to values it never saw in training.
- **A shared SQLite connection under FastAPI's threadpool intermittently corrupted reads.** Concurrent requests reproducibly failed with `TypeError: 'NoneType' object is not subscriptable` on a plain `SELECT COUNT(*)` — sqlite3's `check_same_thread=False` disables its own thread check but doesn't add the locking it was protecting; nothing does that for you. Fixed with an `RLock` around every method that touches the connection (`agent/store.py`), reconfirmed under load: 25 concurrent `POST /forecast` requests for the same tenant, zero errors, exactly 25 new rows persisted.
- **A real rigid-body physics engine turned out to be the wrong tool for "tags fall and never overlap."** Tried Matter.js for the hero's "old way" tags — three tuning rounds in, fixing one failure mode (tags overlapping under solver pressure) reliably reintroduced a worse one (tags flipping onto their edge, then overlap getting *worse* after tuning against that). Replaced the settled layout with a plain CSS flex-wrap — an unconditional overlap guarantee from the browser's own layout engine — and kept a framer-motion spring per tag for the tumble/fall motion. Simpler, and it can't overlap by construction instead of by tuning.
- **A CSS radial mask meant only for a background grid was fading out real content sitting in the same element.** `mask-image` clips an element's entire rendered output, not just its `background-image` — a decorative grid texture and the hero's CTA button/tag row shared a container, so the button faded out at the mask's edge right along with the grid lines. Not caught by any automated check, only by looking at an actual screenshot. Fixed by moving the grid onto its own empty, absolutely-positioned layer with nothing else inside it.
- **A framer-motion + Next.js SSR quirk caused hydration mismatches even after removing the obvious cause.** Swapped `Math.random()` (impure, and different on server vs. client render) for a seeded deterministic function — the mismatch persisted anyway, because framer-motion itself serializes its inline transform/opacity styles with different float precision between the server render and the client mount (`"0"` vs `0`, rounded vs. full-precision). Rendering that component client-only (`next/dynamic`, `ssr: false`) sidesteps it rather than chasing framer-motion's internals.
- **Windows silently corrupts non-ASCII console output when it's redirected, and it doesn't look like a crash.** `agent/wrapper.py`'s own CLI, which prints a `Recommendation`'s description (a legitimate em dash for readability), produced byte-for-byte invalid UTF-8 when its output was piped to a file — Python falls back to the system codepage (cp1252) instead of UTF-8 when stdout isn't an interactive console. Found only during a from-scratch fresh-clone test that checked raw bytes, not visual output; fixed with `sys.stdout.reconfigure(encoding="utf-8")` at each CLI entry point.
